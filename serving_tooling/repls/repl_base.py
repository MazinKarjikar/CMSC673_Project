#!/usr/bin/env python3
"""
Baseline REPL - Uses base Llama 3.1 8B Instruct for BOTH planner and worker.
This is the baseline to compare against the trained planner.
...
class RecursiveReasoner:
    """Recursive reasoning system - BASELINE using same model for planner and worker."""
    ...
    def __init__(
        ...
        self.client = client
        self.model = model  # Same model for both planner and worker
    ...
    def plan(self, problem: str) -> dict:
        """Use model to decide how to approach the problem."""
...
{{
    ...
    "plan": "Brief explanation of approach"
        ...
        response = self.client.generate(prompt, self.model, max_tokens=512)
...
Return JSON: {{"solution": "your calculation", "result": <number>}}"""
        ...
        response = self.client.generate(prompt, self.model, max_tokens=256)
...
Return JSON: {{"final_calculation": "how you combined results", "final_answer": <number>}}"""
    ...
    def solve(self, problem: str) -> str:
        ...
        self.log(f"{Colors.DIM}Using model: {self.model} (baseline){Colors.RESET}")
        ...
        if decompose and plan.get("subproblems"):
            ...
            self.log(f"{Colors.DIM}Using model: {self.model}{Colors.RESET}")
...
┃  🧠  RECURSIVE REASONER (BASELINE)               ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛{Colors.RESET}

{Colors.BLUE}Model:{Colors.RESET}  {reasoner.model}
{Colors.DIM}(Using same base model for planner + worker){Colors.RESET}
...
def main():
    parser = argparse.ArgumentParser(
        description="Baseline REPL - Same model for planner and worker"
    )
    parser.add_argument(
        "--api-base",
        type=str,
