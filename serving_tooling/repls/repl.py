#!/usr/bin/env python3
"""
Interactive REPL for Recursive Reasoner with vLLM backend.

Usage:
    # Start vLLM server first, then run:
    python repl.py
    
    # Or with custom settings:
    python repl.py --planner-model planner --worker-model meta-llama/Llama-3.1-8B-Instruct
    python repl.py --api-base http://localhost:8000/v1
"""
...
class VLLMClient:
    ...
    def generate(
        ...
        # Use chat/completions endpoint - it automatically applies the chat template
        # which includes special tokens like <|begin_of_text|>, <|start_header_id|>, etc.
...
class RecursiveReasoner:
    """Recursive reasoning system with planner and worker models."""
    ...
    def plan(self, problem: str) -> dict:
        """Use planner to decide how to approach the problem."""
        # MUST match training prompt exactly from grpo_training.py prepare_dataset()
...
{{
    ...
    "plan": "Brief explanation of approach"
        ...
        response = self.client.generate(prompt, self.planner_model, max_tokens=512)
    ...
    def solve_subproblem(self, subproblem: str, original_question: str) -> Tuple[str, dict]:
        """Solve a single subproblem. Returns (raw_response, parsed_result)."""
        # MUST match training prompt exactly from grpo_training.py FrozenWorker.solve_subproblem()
...
Return JSON: {{"solution": "your calculation", "result": <number>}}"""
        ...
        response = self.client.generate(prompt, self.worker_model, max_tokens=256)
        parsed = parse_json(response)
        return response.strip(), parsed
    
    def solve_atomic(self, problem: str) -> Tuple[str, dict]:
        """Solve a problem directly without decomposition. Returns (raw_response, parsed_result)."""
        # MUST match training prompt exactly from grpo_training.py FrozenWorker.solve_directly()
...
Return JSON: {{"solution": "your step-by-step solution", "final_answer": <number>}}"""
        ...
        response = self.client.generate(prompt, self.worker_model, max_tokens=512)
        parsed = parse_json(response)
        return response.strip(), parsed
    
    def combine(self, problem: str, subproblems: list, solutions: list) -> Tuple[str, dict]:
        """Combine subproblem solutions into final answer. Returns (raw_response, parsed_result)."""
        # MUST match training prompt exactly from grpo_training.py FrozenWorker.combine_solutions()
        solutions_text = ""
...
Return JSON: {{"final_calculation": "how you combined results", "final_answer": <number>}}"""
    ...
    def solve(self, problem: str) -> str:
        """Main entry point: solve a problem using recursive reasoning."""
        
        # Show problem - ALWAYS show
        print(header("📝 PROBLEM", Colors.WHITE))
        print(f"\n{problem}\n")
        
        # Step 1: Plan
        self.log(header("🧠 PHASE 1: PLANNING", Colors.MAGENTA))
        self.log(f"{Colors.DIM}Using planner model: {self.planner_model}{Colors.RESET}")
        ...
        if decompose and plan.get("subproblems"):
            ...
            # Show subproblems in a nice box
            subproblems_text = "\n".join([
            ...
            self.log(f"{Colors.DIM}Using worker model: {self.worker_model}{Colors.RESET}")
            ...
            for i, sp in enumerate(subproblems, 1):
                ...
                # Show solution in a box - extract result if parsed
            ...
            # Show what we're combining - use parsed results
            combine_preview = "\n\n".join([
            ...
            # Pass raw solutions for combining (worker expects text)
        ...
        # Final answer - ALWAYS show, regardless of verbose mode
    ...
    def _extract_result(self, parsed: Optional[dict], raw: str) -> str:
        """Extract the result from a parsed response or raw text."""
        if parsed:
            if "result" in parsed:
                return str(parsed["result"])
            if "final_answer" in parsed:
                return str(parsed["final_answer"])
        # Fallback: try to find number in raw text
        import re
...
def main():
    parser = argparse.ArgumentParser(
        description="Interactive REPL for Recursive Reasoner"
    )
    parser.add_argument(
        "--api-base",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="planner",
        help="Planner model name (as registered in vLLM)"
    )
    parser.add_argument(
        "--worker-model",
        type=str,
    ...
    # Create client
    client = VLLMClient(args.api_base)
    
    # Check server
    if not check_server(client):
        print("\n❌ Cannot connect to vLLM server!")
        print(f"   Make sure vLLM is running at: {args.api_base}")
        print("\n   Example to start vLLM with LoRA:")
        print("   python -m vllm.entrypoints.openai.api_server \\")
        print("       --model meta-llama/Llama-3.1-8B-Instruct \\")
        print("       --enable-lora \\")
        print("       --lora-modules planner=./adapter \\")
        print("       --port 8000")
        sys.exit(1)
